<template>
  <div class="dishes-page">
    ...
    <el-table :data="dishList" v-loading="loading" class="dish-table">
    ...
    <div class="pagination-bar"><el-pagination v-model:current-page="pageNum" :page-size="pageSize" :total="total" layout="total, prev, pager, next" @current-change="loadDishes" /></div>
...
const dishList = ref([]); const pageNum = ref(1); const pageSize = ref(10)
const total = ref(0); const loading = ref(false); const dialogVisible = ref(false)
...
const loadDishes = async () => {
  ...
  try {
    ...
    if (userStore.userInfo?.shopId) params.shopId = userStore.userInfo.shopId
...
const saveDish = async () => {
  saving.value = true
  try {
    const data = { ...dishForm, shopId: userStore.userInfo?.shopId }
